...
public class ResourceManage extends BaseServlet {
    ...
    public void updateResNodeInfo(HttpServletRequest request, HttpServletResponse response) {
        SqlSession session = DataBaseManage.getSqlSessionFactory().openSession();
        try {
            if (null == request.getParameter("id")) {
                response.getWriter().println(new CommonResult(false, "参数为空", null));
            ...
            res.setName(name);
            res.setCnname(cnname);
            res.setUrlpath(urlpath);
            res.setIstop(istop);
            res.setHaschild(haschild);
            res.setId(id);
            int num = mapper.updateByExample(res);
            if (num >= 0) {
                session.commit();
                response.getWriter().println(new CommonResult(true, "更新成功", null));
                return;
            }
        } catch (Exception e) {
            session.rollback();
        ...
        try {
            response.getWriter().println(new CommonResult(false, "更新失败", null));
            return;
        } catch (Exception e) {
            e.printStackTrace();
        }
