...
public class ResourceSqlProvider {
	public String updateByExample(ResourceTable res) {
		return new SQL() {{
			...
			if (null != res.getName())
				SET("name=#{name}");
			if (null != res.getCnname())
				SET("cnname=#{cnname}");
			if (null != res.getIstop())
				SET("istop=#{istop}");
			if (null != res.getUrlpath())
				SET("urlpath=#{urlpath}");
			if (null != res.isHaschild())
	...
	public String selectByExample(ResourceTable res) {
		return new SQL() {{
			...
			if (null != res.getIstop())
				WHERE("istop=#{istop}");
			if (null != res.getLeftvalue())
				WHERE("leftvalue=#{leftvalue}");
			if (null != res.getRightvalue())
				WHERE("rightvalue=#{rightvalue}");
			if (null != res.getLevel())
				WHERE("level=#{level}");
			if (null != res.getUrlpath())
				WHERE("urlpath=#{urlpath}");
			if (null != res.getOrder())
				WHERE("order=#{order}");
			if (null != res.isHaschild())
				WHERE("haschild=#{haschild}");
		}}.toString();
	}
	...
	public String updateByCondition(@Param("res") ResourceTable res, List<Condition> conds) {
		return new SQL() {{
			...
			if (null != res.getName())
				SET("name=#{res.name}");
			if (null != res.getCnname())
				SET("cnname=#{res.cnname}");
			if (null != res.getIstop())
				SET("istop=#{res.istop}");
			if (null != res.getLeftvalue())
				SET("leftvalue=#{res.leftvalue}");
			if (null != res.getRightvalue())
				SET("rightvalue=#{res.rightvalue}");
			if (null != res.getLevel())
				SET("level=#{level}");
			if (null != res.getOrder())
				SET("order=#{order}");
